...
compiler.compileSync = function (content, filePath) {
  ...
  fragment.childNodes.map(function (node) {
    ...
    switch (node.nodeName) {
      case 'template':
        data = processTemplate(node, filePath, id, hasScopedStyle, content)
        output += ((data ? mergeParts(data, filePath) : '') + '\n')
        break
      case 'style':
        data = processStyle(node, filePath, id)
        output += ((data ? mergeParts(data, filePath) : '') + '\n')
        break
      case 'script':
        data = processScript(node, filePath, content)
        output += ((data ? mergeParts(data, filePath) : '') + '\n')
        break
    }
  })
